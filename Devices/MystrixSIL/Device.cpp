#include "Device.h"
#include "MatrixOS.h"
#include "../../OS/System/ReleaseConfig.h"
...
namespace Device
{
  ...
  void ErrorHandler()
  {
    std::fprintf(stderr, "[MystrixSIL] Error handler invoked\n");
  }
...
EMSCRIPTEN_KEEPALIVE const char* MatrixOS_Wasm_GetVersionString()
{
  return MATRIXOS_VERSION_STRING.c_str();
}

EMSCRIPTEN_KEEPALIVE void MatrixOS_Wasm_Reboot()
{
  Device::Reboot();
}

EMSCRIPTEN_KEEPALIVE void MatrixOS_Wasm_Bootloader()
{
  Device::Bootloader();
}
}
